...
function setup() {
    ...
    window.addEventListener('keydown', preventArrowKeyScroll); // Prevent arrow key scroll during gameplay
    window.addEventListener('keyup', allowArrowKeyScroll); // Allow arrow key scroll when not playing
}
...
function preventArrowKeyScroll(event) {
    if (gameState === "play" && (event.key === "ArrowUp" || event.key === "ArrowDown")) {
        event.preventDefault(); // Prevent arrow keys from scrolling the page during gameplay
    }
}

function allowArrowKeyScroll() {
    // No action needed here, as scrolling will be automatically re-enabled when the game is not playing
}
